        ...
        <Hidden smDown implementation="css">
            <div id="pagination-chef">
                ...
                <ul className="chefList">
                    { 
                      chefs.length > 0 && currentChef.map((chef: any) => (
                        <li key={chef.id} >
                            <img id={`id_${chef.id}`} src={chef.imagePath === null ? foodNotFound : chef.imagePath} alt={chef.name} />                  
                            <span className="text">{chef.name}</span>
                            <span className="rating">
                                <BsFillStarFill color="red"/>
                                <span style={{margin: "0px 5px"}}>{chef.avgRating === null ? "No rating " : chef.avgRating}</span>
                            </span>
                        </li>
                      ))
                    }
